...
class Movies:
	...
	def fetch_list(self):
		...
		try:
			...
			elif self.action == 'fl_movies_related':
				self.id_type = 'fl_dict'
	...
	def build_movie_content(self, _position, _id):
		try:
			...
			browse_related_params = self.build_url({'mode': 'build_movie_list', 'action': 'fl_movies_related', 'key_id': imdb_id, 'is_external': self.is_external,
