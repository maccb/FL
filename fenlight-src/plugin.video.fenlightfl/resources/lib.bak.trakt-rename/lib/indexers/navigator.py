...
class Navigator:
	...
	def my_content(self):
		if s.fl_user_active(): self.add({'mode': 'navigator.fl_lists_personal'}, 'FL Lists', 'trakt')
	...
	def fl_lists_personal(self):
		self.add({'mode': 'navigator.fl_watchlists'}, 'FL Watchlist', 'trakt')
		self.add({'mode': 'fl.list.get_fl_lists', 'list_type': 'my_lists', 'category_name': 'My Lists'}, 'FL My Lists', 'trakt')
		self.add({'mode': 'navigator.fl_favorites', 'category_name': 'Favorites'}, 'FL Favorites', 'trakt')
		self.add({'mode': 'build_my_calendar'}, 'FL Calendar', 'trakt')
		self.end_directory()

	def fl_watchlists(self):
		self.category_name = 'Watchlist'
		self.add({'mode': 'build_movie_list', 'action': 'fl_watchlist', 'category_name': 'Movies Watchlist'}, 'Movies Watchlist', 'trakt')
		self.add({'mode': 'build_tvshow_list', 'action': 'fl_watchlist', 'category_name': 'TV Shows Watchlist'}, 'TV Shows Watchlist', 'trakt')
		self.add({'mode': 'build_movie_list', 'action': 'fl_watchlist_lists', 'new_page': 'recent', 'category_name': 'Recently Added Movies'}, 'Recently Added Movies', 'trakt')
	...
	def fl_favorites(self):
		...
		self.add({'mode': 'build_movie_list', 'action': 'fl_favorites', 'category_name': 'Favorite Movies'}, 'Movies', 'trakt')
		self.add({'mode': 'build_tvshow_list', 'action': 'fl_favorites', 'category_name': 'Favorite TV Shows'}, 'TV Shows', 'trakt')
		self.end_directory()
	...
	def maintenance(self):
		...
		self.add({'mode': 'clear_cache', 'cache': 'trakt', 'isFolder': 'false'}, 'Clear FL Cache', 'settings')
	...
	def changelog_utils(self):
		...
		fenlight_clogpath = k.translate_path('special://home/addons/plugin.video.fenlightfl/resources/text/changelog.txt')
	...
	def certifications(self):
		...
		if menu_type == 'movie': mode, action = 'build_movie_list', 'tmdb_movies_certifications'
			...
			if menu_type == 'tvshow': action = 'fl_tv_certifications'
			else: action = 'fl_anime_certifications'
	...
	def search_history(self):
		...
		'fl_lists': ('fl_list_queries', {'mode': 'search.get_key_id', 'search_type': 'fl_lists', 'isFolder': 'false'})}
	...
	def build_shortcut_folder_contents(self):
		...
		if contents:
			for count, item in enumerate(contents):
				...
				icon = iconImage
				if iconImage:
					if iconImage.startswith('http') or 'plugin.video.fenlightfl' in iconImage: original_image = True
	...
	def tips(self):
		tips_location = 'special://home/addons/plugin.video.fenlightfl/resources/text/tips'
	...
	def because_you_watched(self):
		...
		menu_type = self.params_get('menu_type')
		action_dict = {'movie':
		{'mode': 'build_movie_list', 'action': {0: 'tmdb_movies_recommendations', 1: 'imdb_more_like_this', 2: 'ai_similar', 3: 'fl_movies_related'}, 'media_type': 'movie'},
						'tvshow':
		{'mode': 'build_tvshow_list', 'action': {0: 'tmdb_tv_recommendations', 1: 'imdb_more_like_this', 2: 'ai_similar', 3: 'fl_tv_related'}, 'media_type': 'episode'}}
