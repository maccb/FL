...
def browse_in_fl_list():
	params = get_params('browse_in_fl_list_params')
	if params:
		window_function = activate_window if params['is_external'] in (True, 'True', 'true') else container_update
		return window_function(params)

def fl_manager():
	params = get_params('fl_manager_params')
	if params: dialogs.fl_manager_choice(params)
...
def get_params(param_name):
	try: params = dict(parse_qsl(get_infolabel('ListItem.Property(fenlightfl.%s)' % param_name).split('plugin://plugin.video.fenlightfl/?')[1], keep_blank_values=True))
	except: params = None
	return params