...
def get_key_id(params):
	...
	if search_type == 'media_title':
		...
		elif media_type == 'anime': url_params, string = {'mode': 'build_tvshow_list', 'action': 'fl_anime_search', 'is_anime_list': 'true'}, 'anime_queries'
		elif media_type == 'tvshow_anime': url_params, string = {'mode': 'build_tvshow_list', 'action': 'tmdb_tv_search'}, 'tvshow_anime_queries'
		else: url_params, string = {'mode': 'build_tvshow_list', 'action': 'fl_tv_search'}, 'tvshow_queries'
	...
	elif search_type == 'fl_lists':
		url_params, string = {'mode': 'fl.list.search_fl_lists'}, 'fl_list_queries'
...
def clear_search():
	...
	('Clear FL List Search History', 'fl_list_queries')]
	try:
