...
class FenLightPlayer(xbmc.Player):
	...
	def set_playback_properties(self):
		try:
			fl_ids = {'tmdb': self.tmdb_id, 'imdb': self.imdb_id, 'slug': make_fl_slug(self.title)}
			if self.media_type == 'episode': fl_ids['tvdb'] = self.tvdb_id
			ku.set_property('script.flicklist.ids', json.dumps(fl_ids))
