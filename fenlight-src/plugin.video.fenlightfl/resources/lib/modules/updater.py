...
def get_changes(online_version=None):
	try:
		if not online_version:
			...
			if not version_check(current_version, online_version): return kodi_utils.ok_dialog(heading='FL Updater',
				text='You are running the current version of FL.[CR][CR]There is no new version changelog to view.')
...
def update_check(action=4):
	...
	if not version_check(current_version, online_version):
		...
		return
	if action in (0, 4):
		if not kodi_utils.confirm_dialog(heading='FL Updater', text='Installed Version: [B]%s[/B][CR]Online Version: [B]%s[/B][CR][CR] %s' \
			% (current_version, online_version, '[B]An Update is Available[/B][CR]Perform Update?'), ok_label='Yes', cancel_label='No'): return
		if kodi_utils.confirm_dialog(heading='FL Updater', text='Do you want to view the changelog for the new release before installing?', ok_label='Yes', cancel_label='No'):
			get_changes(online_version)
			if not kodi_utils.confirm_dialog(heading='FL Updater', text='Continue with Update After Viewing Changes?', ok_label='Yes', cancel_label='No'): return
			show_after_action = False
	if action == 1: kodi_utils.notification('FL Update Occuring', icon=kodi_utils.get_icon('downloads'))
	elif action == 2: return kodi_utils.notification('FL Update Available', icon=kodi_utils.get_icon('downloads'))
...
def rollback_check():
	...
	if not kodi_utils.confirm_dialog(heading='FL Updater',
		text='Are you sure?[CR]Version [B]%s[/B] will overwrite your current installed version.[CR]FL will set your update action to [B]OFF[/B] if rollback is successful' \
...
def update_addon(new_version, action, show_after_action=True):
	...
	if action == 5:
		...
		if show_after_action:
			if kodi_utils.confirm_dialog(heading='FL Updater', text='[CR]Success.[CR]FL updated to version [B]%s[/B]' % new_version,
										ok_label='Changelog', cancel_label='Exit', default_control=10) != False:
				kodi_utils.show_text('Changelog', file=kodi_utils.translate_path('special://home/addons/plugin.video.fenlightfl/resources/text/changelog.txt'), font_size='large')
		else:
			kodi_utils.ok_dialog(heading='FL Updater', text='[CR]Success.[CR]FL updated to version [B]%s[/B]' % new_version)
