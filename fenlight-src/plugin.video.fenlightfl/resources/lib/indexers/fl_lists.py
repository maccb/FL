...
from apis.flicklist_api import fl_get_lists, fl_search_lists, get_fl_list_contents, fl_lists_with_media
...
def search_fl_lists(params):
	def _builder():
		for item in lists:
			try:
				...
				list_name, user, list_id = list_info['name'], list_info['username'], list_info['ids']['fl']
				if not list_id: continue
				display = '%s | [I]%s (x%s)[/I]' % (list_name, user, str(item_count))
				url = build_url({'mode': 'fl.list.build_fl_list', 'list_id': list_id, 'list_type': 'user_lists', 'list_name': list_name,
				'iconImage': 'fl', 'name': list_name})
				cm = [('[B]Like List[/B]', 'RunPlugin(%s)' % build_url({'mode': 'fl.fl_like_a_list', 'list_id': list_id})),
				('[B]Unlike List[/B]', 'RunPlugin(%s)' % build_url({'mode': 'fl.fl_unlike_a_list', 'list_id': list_id})),
				('[B]Add to Shortcut Folder[/B]', 'RunPlugin(%s)' % build_url({'mode': 'menu_editor.shortcut_folder_add_known', 'url': url}))]
				listitem = make_listitem()
				listitem.setLabel(display)
				listitem.setArt({'icon': fl_icon, 'poster': fl_icon, 'thumb': fl_icon, 'fanart': fanart, 'banner': fanart})
				info_tag = listitem.getVideoInfoTag(True)
				info_tag.setPlot(' ')
				listitem.addContextMenuItems(cm)
				yield (url, listitem, True)
			except: pass
	handle, search_title, fl_icon, fanart = int(sys.argv[1]), '', kodi_utils.get_icon('fl'), kodi_utils.get_addon_fanart()
	...
	try:
		...
		lists, pages = fl_search_lists(search_title, page)
...
def get_fl_lists(params):
	def get_custom_image(list_name, list_type, user, image_type, images):
		try:
			...
			return os.path.join(profile_path, 'images', 'fl_%s_%s' % (list_type, image_type), custom_image)
		except: return ''
	def _process():
		for item in data:
			try:
				cm = []
				cm_append = cm.append
				list_name, list_id, user, slug, item_count = item['name'], item['ids']['fl'], item['user']['ids']['slug'], item['ids']['slug'], item['item_count']
				...
				mode = 'random.build_fl_lists_contents' if random else 'fl.list.build_fl_list'
				url_params = {'mode': mode, 'user': user, 'slug': slug, 'list_type': list_type, 'list_id': list_id, 'list_name': list_name, 'iconImage': 'fl',
				...
				if list_type == 'liked_lists':
					display = '%s | [I]%s (x%s)[/I]' % (list_name, user, str(item_count))
					cm_append(('[B]Unlike List[/B]', 'RunPlugin(%s)' % build_url({'mode': 'fl.fl_unlike_a_list', 'user': user, 'list_slug': slug})))
				else:
					display = '%s [I](x%s)[/I]' % (list_name, str(item_count))
					cm_append(('[B]Make New List[/B]', 'RunPlugin(%s)' % build_url({'mode': 'fl.make_new_fl_list'})))
					cm_append(('[B]Delete List[/B]', 'RunPlugin(%s)' % build_url({'mode': 'fl.delete_fl_list', 'user': user, 'list_slug': slug})))
				cm_append(('[B]Add to Shortcut Folder[/B]', 'RunPlugin(%s)' % build_url({'mode': 'menu_editor.shortcut_folder_add_known', 'url': url})))
				cm_append(('[B]Set Custom Sort[/B]', 'RunPlugin(%s)' % build_url({'mode': 'fl.list.set_list_custom_sort', 'list_id': list_id,
					'sort_by': sort_by, 'sort_how': sort_how})))
				cm_append(('[B]Make Custom Poster[/B]', 'RunPlugin(%s)' % build_url({'mode': 'fl.list.make_custom_artwork', 'action': 'make_poster',
					'custom_image': custom_poster, 'list_name': list_name, 'list_type': list_type, 'list_id': list_id, 'user': user, 'list_slug': slug})))
				cm_append(('[B]Make Custom Fanart[/B]', 'RunPlugin(%s)' % build_url({'mode': 'fl.list.make_custom_artwork', 'action': 'make_fanart',
					'custom_image': custom_fanart, 'list_name': list_name, 'list_type': list_type, 'list_id': list_id, 'user': user, 'list_slug': slug})))
				if custom_poster: cm_append(('[B]Delete Custom Poster[/B]', 'RunPlugin(%s)' % build_url({'mode': 'fl.list.delete_current_image', 'custom_image': custom_poster})))
				if custom_fanart: cm_append(('[B]Delete Custom Fanart[/B]', 'RunPlugin(%s)' % build_url({'mode': 'fl.list.delete_current_image', 'custom_image': custom_fanart})))
				listitem = make_listitem()
				listitem.setLabel(display)
	...
	handle, fl_icon, fanart = int(sys.argv[1]), kodi_utils.get_icon('fl'), kodi_utils.get_addon_fanart()
	try:
		...
		all_posters = kodi_utils.list_dirs(os.path.join(profile_path, 'images', 'fl_%s_poster' % list_type))[1]
		all_fanart = kodi_utils.list_dirs(os.path.join(profile_path, 'images', 'fl_%s_fanart' % list_type))[1]
		returning_to_list = False
		data = fl_get_lists(list_type)
		all_custom_sorts = get_all_lists_custom_sort()
		if list_type == 'liked_lists': data = [i['list'] for i in data]
		if data:
			if shuffle_lists:
				returning_to_list = 'build_fl_lists_contents' in kodi_utils.folder_path()
...
def get_fl_user_lists(params):
	def _process():
		for _list in lists:
			try:
				...
				list_name, list_id, user, slug = item['name'], item['ids']['fl'], item['user']['ids']['slug'], item['ids']['slug']
				...
				display = '%s | [I]%s (x%s)[/I]' % (list_name, user, str(item_count))
				mode = 'random.build_fl_lists_contents' if random else 'fl.list.build_fl_list'
				url_params = {'mode': mode, 'user': user, 'slug': slug, 'list_id': list_id, 'list_type': 'user_lists', 'list_name': list_name, 'iconImage': 'fl',
								'name': list_name, 'sort_by': sort_by, 'sort_how': sort_how}
				if random: url_params['random'] = 'true'
				url = build_url(url_params)
				listitem = make_listitem()
				if user != 'FL Official':
					cm_append(('[B]Like List[/B]', 'RunPlugin(%s)' % build_url({'mode': 'fl.fl_like_a_list', 'user': user, 'list_slug': slug})))
					cm_append(('[B]Unlike List[/B]', 'RunPlugin(%s)' % build_url({'mode': 'fl.fl_unlike_a_list', 'user': user, 'list_slug': slug})))
				cm_append(('[B]Add to Shortcut Folder[/B]', 'RunPlugin(%s)' % build_url({'mode': 'menu_editor.shortcut_folder_add_known', 'url': url})))
				listitem.addContextMenuItems(cm)
				listitem.setLabel(display)
	...
	try:
		...
		page = params.get('new_page', '1')
		new_page = str(int(page) + 1)
		lists = fl_get_lists(list_type, page)
		kodi_utils.add_items(handle, list(_process()))
		kodi_utils.add_dir(handle, {'mode': 'fl.list.get_fl_user_lists', 'list_type': list_type, 'new_page': new_page},
...
def in_fl_lists(params):
	def _process():
		for item in lists:
			try:
				...
				display = '%s | [I]%s (x%s)[/I]' % (list_name, user, str(item_count))
				url = kodi_utils.build_url({'mode': 'fl.list.build_fl_list', 'user': user, 'slug': slug, 'list_type': 'user_lists', 'list_name': list_name})
				listitem = kodi_utils.make_listitem()
				if not user == 'FL Official':
					cm_append(('[B]Like List[/B]', 'RunPlugin(%s)' % kodi_utils.build_url({'mode': 'fl.fl_like_a_list', 'user': user, 'list_slug': slug})))
					cm_append(('[B]Unlike List[/B]', 'RunPlugin(%s)' % kodi_utils.build_url({'mode': 'fl.fl_unlike_a_list', 'user': user, 'list_slug': slug})))
				listitem.addContextMenuItems(cm)
				listitem.setLabel(display)
				listitem.setArt({'icon': fl_icon, 'poster': fl_icon, 'thumb': fl_icon, 'fanart': fanart, 'banner': fanart})
				info_tag = listitem.getVideoInfoTag(True)
	...
	handle, fl_icon, fanart = int(sys.argv[1]), kodi_utils.get_icon('fl'), kodi_utils.get_addon_fanart()
	try:
		lists = fl_lists_with_media(params['media_type'], params['imdb_id'])
		kodi_utils.add_items(handle, list(_process()))
	except: pass
	kodi_utils.set_content(handle, 'files')
	kodi_utils.set_category(handle, params.get('category_name', 'FL Lists'))
...
def build_fl_list(params):
	def _process(function, _list, _type):
		...
		else: item_list_extend(function('episode.fl_list', _list['list']))
	...
	try:
		...
		if page_no == 1 and not is_external: kodi_utils.set_property('fenlightfl.exit_params', kodi_utils.folder_path())
		if use_result: result = params.get('result', [])
			...
			result = get_fl_list_contents(list_type, user, slug, with_auth, list_id, sort_by, sort_how)
		...
		movie_list = {'list': [(i['order'], i['media_ids']) for i in all_movies], 'id_type': 'fl_dict', 'custom_order': 'true'}
		tvshow_list = {'list': [(i['order'], i['media_ids']) for i in all_tvshows], 'id_type': 'fl_dict', 'custom_order': 'true'}
		...
		new_params = {'mode': 'fl.list.build_fl_list', 'list_type': list_type, 'list_name': list_name, 'user': user, 'slug': slug, 'paginate_start': paginate_start,
						'sort_by': sort_by, 'sort_how': sort_how}
...
def make_custom_artwork(params):
	...
	new_image = fl_image_maker(list_name, list_type, list_id, image_type, user, slug, custom_image, shuffle_art)
	kodi_utils.kodi_refresh()

def fl_image_maker(list_name, list_type, list_id, image_type, user, slug, custom_image, shuffle_sort_order):
	...
	content = get_fl_list_contents(list_type, user, slug, True, list_id, sort_by, sort_how)
	...
	final_image = make_image('fl_%s' % list_type, image_type, '%s_%s' % (list_name, user), images, custom_image)
