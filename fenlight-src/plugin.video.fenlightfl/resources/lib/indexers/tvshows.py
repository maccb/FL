...
class TVShows:
	...
	def fetch_list(self):
		...
		try:
			...
			if page_no == 1 and not self.is_external:
				...
				if not any([x in folder_path for x in ('build_season_list', 'build_episode_list')]): kodi_utils.set_property('fenlightfl.exit_params', folder_path)
			...
			elif self.action in self.fl_search:
				key_id = self.params_get('key_id', None) or self.params_get('query')
				if not key_id: return
				self.id_type = 'fl_dict'
			...
			elif self.action == 'fl_tv_related':
				self.id_type = 'fl_dict'
	...
	def build_tvshow_content(self, _position, _id):
		try:
			...
			browse_related_params = self.build_url({'mode': 'build_tvshow_list', 'action': 'fl_tv_related', 'key_id': imdb_id, 'is_external': self.is_external,
			...
			fl_manager_params = self.build_url({'mode': 'fl_manager_choice', 'tmdb_id': tmdb_id, 'imdb_id': imdb_id, 'tvdb_id': tvdb_id, 'media_type': 'tvshow', 'icon': poster})
			...
			if imdb_id:
				browse_in_fl_list_params = self.build_url({'mode': 'fl.list.in_fl_lists', 'media_type': 'tvshow', 'imdb_id': imdb_id, 'is_external': self.is_external,
			...
			set_properties({
				...
				'fenlightfl.browse_in_fl_list_params': browse_in_fl_list_params,
				'fenlightfl.fl_manager_params': fl_manager_params,
